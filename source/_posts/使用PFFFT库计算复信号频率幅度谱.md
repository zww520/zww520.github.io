---
title: 使用PFFFT库计算复信号频率幅度谱
date: 2026-06-14 19:30:26
categories:
    - 信号处理
tags: 
    - 信号处理
...
将正弦波实信号变为IQ信号， IQ信号的核心是用两个正交分量（相位相差90度）来表示一个复数信号
...
- \(I(t)\)：同相分量（In-phase），实部
- \(Q(t)\)：正交分量（Quadrature），虚部
- \(j = \sqrt{-1}\)

IQ信号极坐标表示：
$$
xs(t) = A(t) \cdot e^{j\phi(t)} = A(t) \cdot \left[ \cos\phi(t) + j \cdot \sin\phi(t) \right]
$$
希尔伯特变换在频域中是一个90度移相器，因此可以通过希尔伯特变换将实信号变成IQ信号

对于实信号 \(x(t)\)，其希尔伯特变换定义为：
$$
...
- I路 = \(x(t)\)（原始实信号）
- Q路 = \(j \cdot \hat{x}(t)\)（希尔伯特变换结果）

希尔伯特变化实现：

```cpp
void hilbertTransform(double* buffer, int length, double* complexBuffer)
{
    PFFFTD_Setup* setup = pffftd_new_setup(length, PFFFT_REAL); // 创建PFFFT设置
    double* inputBuffer = (double*)pffftd_aligned_malloc(length * sizeof(double) * 2);
    double* outputBuffer = (double*)pffftd_aligned_malloc(length * sizeof(double) * 2);
    double* workBuffer = (double*)pffftd_aligned_malloc(length * sizeof(double) * 2);

    for (int i = 0; i < length; i++) {
        inputBuffer[i] = buffer[i]; // 复制输入信号
    }

    pffftd_transform_ordered(setup, inputBuffer, outputBuffer, workBuffer, PFFFT_FORWARD); // 执行FFT

    int half = length / 2;
    for (int i = 0; i <= half; i++) { // 加倍正频率分量 
        if (i == 0) {
            outputBuffer[0] *= 1.0; // DC分量保持不变
        } else if (i == half) {
            outputBuffer[1] *= 1.0; // Nyquist分量保持不变
        } else { 
            outputBuffer[2 * i] *= 2.0;
            outputBuffer[2 * i + 1] *= 2.0;
        }
    }
    for (int i = half + 1; i < length; i++) { // 负频率分量置零
        outputBuffer[2 * i] *= 0.0;
        outputBuffer[2 * i + 1] *= 0.0;
    }

    PFFFTD_Setup* ifftSetup = pffftd_new_setup(length, PFFFT_COMPLEX); // 创建逆FFT设置

    pffftd_transform_ordered(ifftSetup, outputBuffer, inputBuffer, workBuffer, PFFFT_BACKWARD); // 执行逆FFT

    for (int i = 0; i < length; i++) {
        complexBuffer[2 * i] = inputBuffer[2 * i] / length;     // 实部归一化
        complexBuffer[2 * i + 1] = inputBuffer[2 * i + 1] / length; // 虚部归一化
    }
    ...
    pffftd_destroy_setup(ifftSetup); // 销毁逆FFT设置
}
```
...
double meanQ = 0.0;
for (int i = 0; i < length; i++) {
    meanI += buffer[2*i]; // 计算平均值
    meanQ += buffer[2*i + 1];
}
meanI /= length; // 平均值
meanQ /= length; // 平均值
for (int i = 0; i < length; i++) {
    inputBuffer[2*i] = buffer[2*i] - meanI; // 去掉直流分量
    inputBuffer[2*i + 1] = buffer[2*i + 1] - meanQ; // 去掉直流分量
}
```
...
```cpp
double applyHanningWindowComplex(double* buffer, int length)
{
    double sum = 0.0;
    for (int i = 0; i < length; i++) {
        double w = 0.5 * (1.0 - std::cos(2.0 * M_PI * i / (length - 1))); // 汉宁窗函数
        buffer[2 * i] *= w;     // 实部乘以窗函数
        buffer[2 * i + 1] *= w; // 虚部乘以窗函数
        sum += w;
    }
...
```cpp
void calculateComplexSignalAmplitudeSpectrum(double* buffer, int length, double* spectrum)
{
    PFFFTD_Setup* setup = pffftd_new_setup(length, PFFFT_COMPLEX); // 创建PFFFT设置
    double* inputBuffer = (double*)pffftd_aligned_malloc(length * sizeof(double) * 2);
    double* outputBuffer = (double*)pffftd_aligned_malloc(length * sizeof(double) * 2);
    double* workBuffer = (double*)pffftd_aligned_malloc(length * sizeof(double) * 2);
    ...
    windowGain = applyHanningWindowComplex(inputBuffer, length);
    ...
    for (int i = 0; i < length; i++) {
        ...
        spectrum[i] = std::sqrt(real * real + imag * imag) / length * windowGain; // 单边幅度并补偿窗函数
    }
    int mid = length / 2;
    for(int i=0;i<mid;i++) {
        std::swap(spectrum[i], spectrum[mid+i]);
    }
    pffftd_aligned_free(inputBuffer); // 释放内存
    pffftd_aligned_free(outputBuffer);
    pffftd_aligned_free(workBuffer);
...
int main(int argc, char* argv[]) {
    ...
    double* complexSignal = new double[length * 2]; // 分配复数信号内存
    double* spectrum = new double[length]; // 分配频谱内存
    generateSineWave(realSignal, length, frequency, samplerate, amplitude, phase); // 生成正弦波
    hilbertTransform(realSignal, length, complexSignal); // 将输入通过Hilbert变换转换为IQ复数信号
    calculateComplexSignalAmplitudeSpectrum(complexSignal, length, spectrum); // 计算幅度谱
...
- 复信号频率的幅度值还原时，不需要除以2
- 对于实信号，能还原的信号频率不超过采样率fs的一半
- 对于实信号，能表示的带宽为\(fs/2\)，范围为\([0, fs/2]\)
- 对于复信号，能还原的信号频率不超过采样率fs的一半
